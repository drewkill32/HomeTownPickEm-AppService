...
public class UserController : ApiControllerBase
{
    ...
    [AllowAnonymous]
    public async Task<ActionResult<TokenDto>> ResendInvite(ResendInvite.Command command)
    {
        await Mediator.Send(command);
        return new StatusCodeResult(StatusCodes.Status201Created);
    }
    ...
    [AllowAnonymous]
    public async Task<ActionResult> ResetPassword(UnSafelyResetPassword.Command command)
    {
        await Mediator.Send(command);
        return Ok();
    }
